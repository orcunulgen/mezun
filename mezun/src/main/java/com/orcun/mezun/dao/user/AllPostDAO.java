...
public class AllPostDAO {
	...
	public List<PostHistory> postHistoryList(User user) {
		...
		return getSession()
		.createCriteria(PostHistory.class)
	...
	/*@SuppressWarnings("unchecked")
	public List<Post> allTextTypePosts(User loggedUser) {
			...
			if (chatGroupList.size() != 0) {
				for (int i = 0; i < chatGroupList.size(); i++) {
					...
					for (int j = 0; j < chatPersonList.size(); j++) {
						postHistories.addAll(getSession()
								.createCriteria(PostHistory.class)
								.add(Restrictions.eq("user", chatPersonList
										.get(j).getUser())).list());
					}
				}
			}
		...
		for (int i = 0; i < postHistories.size(); i++) {
			...
			if (currentPostHistory.getContentType().equals(ContentType.PHOTO)) {
				...
				temp.setContentType(currentPostHistory.getContentType());
				try {
					temp.setDescription(currentPostHistory.getDescription());
		 ...
		 * query.addEntity("ph", PostHistory.class); return query.list();
		 */

	//}

	/*public List<Post> myTextTypePosts(User loggedUser) {
		Criteria cr = getSession().createCriteria(PostHistory.class);
		...
		for (int i = 0; i < cr.list().size(); i++) {
			PostHistory currentPostHistory = (PostHistory) cr.list().get(i);
			...
			if (currentPostHistory.getContentType().equals(ContentType.PHOTO)) {
				...
				myPosts.add(currentPostHistory);
			}
